...
public class BufferedWriter extends Writer {
     ...
     * flushing the stream itself. This method is non-private only so that it may be
     ...
     * underlying stream. Thus redundant <code>BufferedWriter</code>s will not copy
     * data unnecessarily.
     *
    ...
    public void write(char cbuf[], int off, int len) throws IOException {
        synchronized (lock) {
            ...
            if (len >= nChars) {
                /*
                 * If the request length exceeds the size of the output buffer, flush the buffer
