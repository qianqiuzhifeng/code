 ...
 * methods and fields. (FilterInputStream包含一些其他输入流，它将其用作其基本数据源，可能会沿途转换数据或提供其他功能。
 * FilterInputStream类只是简单的重写了InputStream所有方法。FilterInputStream的子类可以重写部分方法或者新增额外的方法和属性。)
 * 
...
public class FilterInputStream extends InputStream {
     ...
     * <code>FilterInputStream</code> depend on the implementation strategy actually
     * used.
     *
     ...
     * this input stream. The next caller might be the same thread or another
     ...
     * ahead a little to see what's in the stream. Often this is most easily done by
     * invoking some general parser. If the stream is of the type handled by the
     * parse, it just chugs along happily. If the stream is not of that type, the
     * parser should toss an exception when it fails. If this happens within
     * readlimit bytes, it allows the outer code to reset the stream and try another
     * parser.
     *
     * @exception IOException if the stream has not been marked or if the mark has
     *                        been invalidated.
     * @see java.io.FilterInputStream#in
     ...
     *
     * @return <code>true</code> if this stream type supports the <code>mark</code>
     *         and <code>reset</code> method; <code>false</code> otherwise.
     * @see java.io.FilterInputStream#in
