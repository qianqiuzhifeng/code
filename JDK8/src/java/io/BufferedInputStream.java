 ...
 * <code>mark</code> and <code>reset</code> methods. （向另一个输入流添加功能 -
 * 即缓冲输入并支持标记和重置方法的能力） When the <code>BufferedInputStream</code> is created, an
 * internal buffer array is created. （当创建BufferedInputStream的同时，创建一个内部缓存数组）As
 * bytes from the stream are read or skipped, the internal buffer is refilled as
 * necessary from the contained input stream, many bytes at a
 * time.（当读取或跳过来自流的字节时，内部缓冲区根据需要从包含的输入流中一次重新填充多个字节。） The <code>mark</code>
 * operation remembers a point in the input stream and the <code>reset</code>
 * operation causes all the bytes read since the most recent <code>mark</code>
 * operation to be reread before new bytes are taken from the contained input
 * stream.（mark操作会记住输入流中的一个点，reset操作会导致自最近的mark操作以来读取的所有字节被重读 从包含的输入流中获取新字节。）
 * 
...
public class BufferedInputStream extends FilterInputStream {
    ...
    /**
     * The maximum size of array to allocate.（分配最大缓冲长度） Some VMs reserve some header
     * words in an array.（一些虚拟机在数组中保留关键字） Attempts to allocate larger arrays may
     * result in OutOfMemoryError: Requested array size exceeds VM
     * limit（尝试分配一个较大的数组会导致OutOfMemoryError：请求的数组长度超过虚拟机的限制时）
     */
    private static int MAX_BUFFER_SIZE = Integer.MAX_VALUE - 8;

    /**
     * The internal buffer array where the data is stored.（存储内部缓冲数组的地方） When
     * necessary, it may be replaced by another array of a different
     * size.（必要的时候，可以被其他不同长度的数组替换）
     */
    protected volatile byte buf[];

    /**
     * Atomic updater to provide compareAndSet for buf.（为CAS提供原子更新） This is
     * necessary because closes can be asynchronous.（这是必须的因为close是异步的） We use
     * nullness of buf[] as primary indicator that this stream is
     * closed.（我们使用buf[]的null值作为此流关闭的主要指示符） (The "in" field is also nulled out on
     * close.)
     */
     ...
     * buffer.（比缓冲区中最后一个有效字节的索引大1的索引。） This value is always in the range
     * <code>0</code> through <code>buf.length</code>;（这个值是0和缓冲长度值之间的随机数） elements
    ...
    /**
     * The current position in the buffer.（当前缓冲的地址） This is the index of the next
     * character to be read from the <code>buf</code> array.（这是下一个将从缓冲数组读取的字符的索引）
     * <p>
     * This value is always in the range <code>0</code> through
     * <code>count</code>.（这个值是0和缓冲长度值之间的随机数） If it is less than <code>count</code>,
     * then <code>buf[pos]</code> is the next byte to be supplied as input; if it is
     ...
     * contained input
     * stream.（如果这个值小于缓冲大小，那么buf[pos]返回输入的下一个字节；如果它等于缓冲大写，那么下一次read或者skip操作就会从输入流中获取更多的字节。）
     *
     ...
     * <code>mark</code> method was called.（这个值是上一次mark方法调用时的pos属性值）
     * <p>
     * This value is always in the range <code>-1</code> through
     * <code>pos</code>.（这个值是-1到pos的一个随机值） If there is no marked position in the
     * input stream, this field is <code>-1</code>.（如果在输入流中没有标记位置，那么这个值就是-1） If
     * there is a marked position in the input stream, then
     * <code>buf[markpos]</code> is the first byte to be supplied as input after a
     * <code>reset</code> operation.（如果输入流中有标记位置，那么buf[markpos]是在reset操作后的一个字节） If
     * <code>markpos</code> is not <code>-1</code>, then all bytes from positions
     * <code>buf[markpos]</code> through <code>buf[pos-1]</code> must remain in the
     * buffer array (though they may be moved to another place in the buffer array,
     * with suitable adjustments to the values of <code>count</code>,
     * <code>pos</code>, and <code>markpos</code>); they may not be discarded unless
     * and until the difference between <code>pos</code> and <code>markpos</code>
     ...
     * （在后续调用reset方法失败之前调用mark方法后允许的最大值）Whenever the difference between
     * <code>pos</code> and <code>markpos</code> exceeds <code>marklimit</code>,
     * then the mark may be dropped by setting <code>markpos</code> to
     * <code>-1</code>.
     *
     * @see java.io.BufferedInputStream#mark(int)
     ...
     * to close; if not return it; (检查以确定当前输入流并未因为流关闭而变成null,如果没有则返回。)
     */
     ...
     * and stored in
     * <code>buf</code>.（创建一个BufferedInputStream对象，并且保存之后使用的InputStream参数。会创建一个内部数组保存缓冲数据。）
     *
     ...
     * <code>buf</code>.（创建一个BufferedInputStream对象，并且指定缓存大小和InputStream参数。会创建一个内部数组保存缓冲数据。）
     *
     * @param in   the underlying input stream.
     ...
     * read in, hence pos > count.（使用更多数据填充缓冲区，考虑到混乱和其他处理标记的技巧。 假设它是由同步方法调用的。
     * 此方法还假定已读入所有数据，因此pos>count。）
     */
    private void fill() throws IOException {
        byte[] buffer = getBufIfOpen(); // 缓存字节数组
        if (markpos < 0)
